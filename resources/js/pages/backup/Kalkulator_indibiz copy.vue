...
// Tambah ke transaksi 
// State untuk keranjang belanja (sesuaikan dengan logika kalkulator Anda)
const cart = ref<any[]>([]);
...
const tambahTransaksi = async () => {
    ...
    // 2. Siapkan data sesuai kebutuhan backend di ProdukController
    const payload = {
        total_harga: grandTotal.value, // Hitungan total dari semua item
        items: cart.value.map(item => ({
            id_produk: item.id_produk, // Pastikan ID Produk sesuai format A-xxxx atau I-xxxx
            kuantitas: item.kuantitas,
            harga_produk: item.harga_produk,
            subtotal: item.kuantitas * item.harga_produk
        }))
    };

    try {
        // 3. Kirim ke endpoint simpanTransaksi di backend
        const response = await axios.post('/api/produk', payload);

        // Sesuai kode controller Anda yang memicu alert backend
        alert("Transaksi Berhasil Ditambahkan");

        // 4. Reset keranjang setelah sukses
        cart.value = [];
        // Jika Anda memiliki fungsi fetch untuk memperbarui list laporan, panggil di sini
        // fetchTransactions(); 
    ...
    <Sidebar>
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold">Kalkulator Transaksi</h2>
                        ...
                        <tbody>
                            <tr v-for="(item, index) in keranjang" :key="item.id_produk">
                                ...
                                <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">{{
                                    formatCurrency(item.harga_produk) }}</td>
                                <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                                    <div class="flex items-center justify-center space-x-2 p-4">
                                        ...
                                        <input type="number" v-model="item.kuantitas"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center space-x-1">
                                        <input type="number" v-model="item.durasiAngka"
                            ...
                            <tr v-if="keranjang.length > 0" class="bg-blue-50/50 font-bold">
                                <td colspan="9" class="px-6 py-4 text-right text-sm tracking-wider uppercase">Total
    ...
    <div v-if="showModal"
        ...
        <div class="my-auto flex max-h-[95vh] w-full max-w-[95vw] flex-col rounded-lg bg-white shadow-xl sm:max-w-md">
            ...
            <div class="flex-1 space-y-3 overflow-y-auto p-4 sm:space-y-4 sm:p-6">
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-medium sm:mb-2 sm:text-sm">Nama Produk</label>
                    <input v-model="searchQuery" type="text" @focus="showDropdown = true" @blur="showDropdown = false"
                        class="w-full rounded-lg border px-2.5 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none sm:px-3 sm:py-2 sm:text-base"
                        placeholder="Cari Produk" />
                    ...
                    <div v-if="showDropdown" class="mt-1 max-h-60 overflow-y-auto rounded border bg-white shadow-md">
                        <div @mousedown.prevent="selectProduk(item)" v-for="item in filteredProduk"
                            ...
                            class="cursor-pointer border-b p-3 transition-colors hover:bg-blue-50">
                            <div class="text-sm font-medium">{{ item.nama_produk }}</div>
                            <div class="text-xs text-gray-500">{{ item.kategori }} - {{
                ...
                <div>
                    ...
                    <input disabled v-model.number="formData.harga_produk" type="number"
                        class="w-full rounded-lg border px-2.5 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none sm:px-3 sm:py-2 sm:text-base"
                ...
                <div>
                    ...
                    <input disabled v-model.number="formData.stok" type="number"
                        class="w-full rounded-lg border px-2.5 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none sm:px-3 sm:py-2 sm:text-base"
